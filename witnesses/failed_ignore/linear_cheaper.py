...
def diagnose_stabilizers(n_qubits, backend, shots=8000, plot=False):
    ...
    if plot:
        ...
        plt.bar(
            indices_sorted,
            ki_sorted,
            color=["steelblue" if v > 0.7 else "salmon" for v in ki_sorted],
        )
        plt.axhline(
            y=0.7, color="red", linestyle="--", label='Typical "good" threshold'
        )
...
def wilson_interval(successes, total, confidence=0.95):
    ...
    margin = (
        z * np.sqrt((p_hat * (1 - p_hat) / total + z**2 / (4 * total**2))) / denominator
    )

    return p_hat, max(0, center - margin), min(1, center + margin)


def run_subchain_witness_theorem_2(
    n_qubits, counts_A, counts_B, shots, start_qubit, end_qubit, confidence_level=0.95
):
    """
    ...
    for bitstring, count in counts_A.items():
        ...
        for i in range(start_qubit, end_qubit + 1, 2):
            ...
            if i > start_qubit and bitstring[n_qubits - 1 - (i - 1)] == "1":
                parity *= -1
            # Right neighbor (only check if within sub-chain)
            if i < end_qubit and bitstring[n_qubits - 1 - (i + 1)] == "1":
                parity *= -1
    ...
    p_A, p_A_lower, p_A_upper = wilson_interval(
        success_count_A, shots, confidence_level
    )
    p_B, p_B_lower, p_B_upper = wilson_interval(
        success_count_B, shots, confidence_level
    )
...
if __name__ == "__main__":
    ...
    noise_model = create_noise_model(
        single_qubit_error=0.001, two_qubit_error=0.02, readout_error=0.002
    )
    ...
    for n in [10, 12, 14, 16, 18, 20]:
        ...
        ki_vals, counts_A, counts_B = diagnose_stabilizers(
            n, backend_sim, shots=8000, plot=False
        )

        # Find best contiguous sub-chain
        best_start, best_end, best_length = find_best_subchain(
            ki_vals, min_threshold=0.7
        )
        ...
        if best_length >= 4:  # Need at least a few qubits for meaningful entanglement
            ...
            result = run_subchain_witness_theorem_2(
                ...
                counts_B,
                shots=8000,
                start_qubit=best_start,
                end_qubit=best_end,
                confidence_level=0.95,
            )

            print(
                f"N={n}: Best sub-chain [{best_start}, {best_end}] ({result['sub_chain_size']} qubits)"
            )
            print(
                f"  W = {result['witness']:.4f}, W_upper(95%) = {result['witness_upper']:.4f}"
            )
            print(f"  P_A = {result['p_A']:.4f}, P_B = {result['p_B']:.4f}")
            if result["certified"]:
                print(
                    f"  ✓ GME CERTIFIED at 95% confidence over {result['sub_chain_size']} qubits"
                )
            else:
